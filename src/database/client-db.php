...
function getClient($email_utilisateur): array
{
    $pdo = getConnexion();
    $requete_email = $pdo->prepare("SELECT * FROM client WHERE email_utilisateur=?");
    $requete_email->execute([$email_utilisateur]);
    $user = $requete_email->fetchAll(PDO::FETCH_ASSOC);
    return $user;
}
function postClient($pseudo_utilisateur, $nom_utilisateur, $prenom_utilisateur, $adresse_utilisateur, $email_utilisateur, $mdp_utilisateur): void
{
    $pdo = getConnexion();
    $requete = $pdo->prepare(query: "INSERT INTO client (pseudo,nom_client, prenom_client, adresse, mail, mdp) VALUES (?, ?, ?, ?, ?, ?)");

    $requete->bindParam(1, $pseudo_utilisateur);
    $requete->bindParam(2, $nom_utilisateur);
    $requete->bindParam(3, $prenom_utilisateur);
    $requete->bindParam(4, $adresse_utilisateur);
...
{
    $pdo = getConnexion();
    $requete_mdp = $pdo->prepare("SELECT mdp FROM client WHERE mail=?");
...
{
    $pdo = getConnexion();
    $query = "SELECT mdp FROM client WHERE mail = ?";
...
function InscriptionClient($pseudo_utilisateur, $nom_utilisateur, $prenom_utilisateur, $adresse_utilisateur, $email_utilisateur, $mdp_utilisateur, $mdp_utilisateurCheck, $pdo) {
    ...
    if (empty($prenom_utilisateur)) {
        $erreurs['prenom_utilisateur'] = "Le prenom est obligatoire";
    }
    if (empty($adresse_utilisateur)) {
        $erreurs['adresse_utilisateur'] = "L'adresse est obligatoire";
    }
    ...
    if (empty($erreurs)) {
        ...
        $query = "INSERT INTO  client (pseudo,nom_client,prenom_client,adresse, mail, mdp) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($query);
        $stmt->execute([$pseudo_utilisateur,$nom_utilisateur ,$prenom_utilisateur,$adresse_utilisateur,$email_utilisateur, $hashedPassword]);
...
{
    $pdo = getConnexion();
    $query = "SELECT pseudo FROM client WHERE mail = ?";
    $stmt = $pdo->prepare($query);
    $stmt->execute([$email]);
...
{
    $pdo = getConnexion();
    $requete_email = $pdo->prepare("SELECT * FROM client WHERE mail = ?");
    $requete_email->execute([$email_utilisateur]);
...
{
    $pdo = getConnexion();
    $requete_pseudo = $pdo->prepare("SELECT * FROM client WHERE pseudo = ?");
